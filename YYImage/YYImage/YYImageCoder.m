...
static yy_png_info *yy_png_info_create(const uint8_t *data, uint32_t length) {
    ...
    do {
        ...
        if ((uint64_t)chunk->offset + (uint64_t)chunk->length + 12 > length) goto fail;
        ...
        if (chunk_num == 0 && chunk->fourcc != YY_FOUR_CC('I', 'H', 'D', 'R') ||
            offset == length && chunk->fourcc != YY_FOUR_CC('I', 'E', 'N', 'D')) {
            goto fail;
        }
        ...
        switch (chunk->fourcc) {
            case YY_FOUR_CC('a', 'c', 'T', 'L') : {
                if (apng_frame_number == -1 && first_IDAT_index == 0 && chunk->length == 8) {
            ...
            case YY_FOUR_CC('f', 'd', 'A', 'T') : {
                if (chunk->fourcc == YY_FOUR_CC('f', 'c', 'T', 'L')) {
                    if ((apng_frame_index > 0 ? encountered_fdAT : 1) && chunk->length == 26) {
                        apng_frame_index++;
            ...
            case YY_FOUR_CC('I', 'H', 'D', 'R') : {
                if (chunk->length != 13 || chunk_num != 1) goto fail; 
            } break;
            case YY_FOUR_CC('I', 'E', 'N', 'D') : {
                if (chunk->length != 0) goto fail;
                offset = length; // end, break do-while loop
            } break;
        }
    } while (offset + 12 <= length);
    
    if (first_IDAT_index == 0 || offset != length) goto fail;
    if (!encountered_fdAT) apng_chunk_error = true;
    
    // png info
    info = calloc(1, sizeof(yy_png_info));
    if (!info) goto fail;
    info->chunks = chunks;
    ...
    if (!apng_chunk_error && apng_frame_number == apng_frame_index && apng_frame_number >= 1) {
        ...
        if (!info->apng_frames) goto fail;
        info->apng_shared_chunk_indexs = calloc(info->chunk_num, sizeof(uint32_t));
        if (!info->apng_shared_chunk_indexs) goto fail;
...
fail:
    if (info) yy_png_info_release(info);
    else if (chunks) free(chunks);
    return NULL;
}
    ...
    *size = frame_remux_size;
    ...
    for (uint32_t i = 0; i < info->apng_shared_chunk_num; i++) {
        ...
        if (shared_chunk_index >= info->apng_shared_insert_index && !inserted) { // replace IDAT with fdAT
            inserted = true;
            yy_png_chunk_info *insert_chunk_info = info->chunks + frame_info->chunk_index;
            for (uint32_t c = frame_info->chunk_num; c > 0; insert_chunk_info++) {
                if (insert_chunk_info->fourcc == YY_FOUR_CC('f', 'd', 'A', 'T')) {
                    ...
                    *((uint32_t *)(frame_data + data_offset + insert_chunk_info->length + 4)) = yy_swap_endian_uint32(crc);
                    ...
                    c--;
                } else if (insert_chunk_info->fourcc == YY_FOUR_CC('I', 'D', 'A', 'T')) {
